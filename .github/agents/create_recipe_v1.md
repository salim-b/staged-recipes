...
- Make sure to add the correct `homepage` URL (different from the `repository` URL) if such an official website exists. Also add an additional `documentation` URL if a dedicated one exists.
