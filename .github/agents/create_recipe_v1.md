...
- DO NOT add `schema_version: 1` since this is the default anyways.
...
- Ensure the created recipe builds successfully using `rattler-build build --recipe recipes/<recipe name>/recipe.yaml -m .ci_support/linux64.yaml`.
  ...
  If it isn't already installed, download the latest `rattler-build` binary from <https://github.com/prefix-dev/rattler-build/releases/latest/download/rattler-build-x86_64-unknown-linux-musl.tar.gz>.
