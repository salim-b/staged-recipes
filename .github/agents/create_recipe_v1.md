...
Create a conda-forge [v1 build recipe](https://rattler-build.prefix.dev/latest/reference/recipe_file/) for the provided CLI tool. Name the package after the CLI command unless instructed otherwise. Base your changes on the `origin/main` Git branch and name the new branch according to the following pattern: `copilot/conda-forge/add-<recipe name>`
...
- `pkg-config`-discovered native libraries needed at link/compile time must go in `requirements.host`, not `requirements.build`.
...
- If a CLI requires a newer glibc version than conda-forge's default 2.17, you can [opt in](https://conda-forge.org/docs/maintainer/knowledge_base/#requiring-newer-glibc-versions) to version `2.28`, `2.34` or `2.39` by setting `c_stdlib_version` in `recipes/<recipe name>/conda_build_config.yaml`:
  ...
  Note that you also need to adapt `c_stdlib_version` in `.ci_support/linux64.yaml` for local builds to use the newer glibc version (but NEVER *commit* changes to `.ci_support/*`).
...
- DO NOT open a PR when finished.
